...
namespace _2048WinFormsApp
{
    public partial class MainForm : Form
    {
        ...
        private void GenerateNumber()
        {
            ...
            {
                var message = $"Èãðà çàêîí÷åíà!\nÂû ïðîèãðàëè.\nÂàø ðåçóëüòàò: {result.Score}";
                if (Win())
                {
                    message = $"Ïîçäðàâëÿþ!\nÂû ïîáåäèëè.\nÂàø ðåçóëüòàò: {result.Score}";
                ...
                MessageBox.Show(message, "Êîíåö èãðû!", MessageBoxButtons.OK);
            }
        }
        private void ShowScore()
        {
            maxScoreLabel.Text = $"Ëó÷øèé ñ÷¸ò: {maxScore}";
            scoreLabel.Text = $"Ñ÷¸ò: {result.Score}";
            if (maxScore < result.Score)
            {
                maxScoreLabel.Text = $"Ëó÷øèé ñ÷¸ò: {result.Score}";
        ...
        {
            DataProvider.ShowText(DataProvider.Rules, "Ïðàâèëà");
        }
        private void ñòàòèñòèêàToolStripMenuItem_Click(object sender, EventArgs e)
        {
            new ResultsForm().ShowDialog();
        }
        private void ðàçìåðûÏîëÿToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var optionSizeForm = new OptionSizeForm();
            optionSizeForm.ShowDialog();
        }
